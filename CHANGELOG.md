...
### 新增
- 全局技能包：实例级可复用技能集合，支持挂载到专家、从 SkillHub 导入技能集，以及本地 ZIP / URL 导入技能 (#108)
- 个性化页整合技能 / 子专家 / 频道 / MBTI / 记忆；技能包管理页支持移动端列表详情切换 (#108)
- 飞书 / 企业微信官方 CLI 连接器：支持安装、设备码用户授权，以及隔离 CONFIG_DIR (#119)
- 新增 OpenCode Zen / Go 预设提供商 (#100)

### 修复
- 挂载技能包时热同步，避免整机 Agent rebuild 导致专家页短暂「未找到 Agent」(#108)
- 安装向导密码文件兼容 `Password: <value>` 标注格式（与部署脚本一致），同时保留纯文本首行回退 (#101)
