...
| 版本 | 包名 | 体积 | 运行方式 | 依赖 |
|------|------|------|----------|------|
| **Docker 版** | `octop-<ver>.fpk` | ~8 KB | 飞牛自动从 `jubaoliang/octop:vanilla` 拉镜像运行 | 宿主需有 Docker 运行时 |
...
├── app/
│   ├── docker/
│   │   └── docker-compose.yaml   # 引用 jubaoliang/octop:vanilla
│   └── ui/
│       ├── config                # 桌面图标入口
...
2. **镜像构建**：`.github/workflows/zz-build-fpk.yml` 的 `image` job 在 `main` 更新时，用 `fnos/Dockerfile` 从仓库源码构建 Octop 镜像并推送到 `jubaoliang/octop:vanilla`（含全部附加组件）。
...
4. Docker 版镜像首次会从 `jubaoliang/octop:vanilla` 拉取；请确保该镜像为公开（workflow 已自动设为 public）。本地版无需联网拉镜像，首次启动会按需要补装 Chromium 系统库（需 root 权限，已尽力处理）。
